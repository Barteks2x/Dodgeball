...
public class Spawn {
    ...
    public boolean spawn(CommandSender sender, Iterator<String> args) {
        ...
        if(!args.hasNext()) {
            mm.setGlobalSpawn(((Entity)sender).getLocation());
            sender.sendMessage(AQUA + "Global dodgeball spawn set: " + ((Entity)sender).
                    getLocation().toString());
            return true;
        }
        String name = args.next();
        mm.getMinigame(name).setSpawn(((Entity)sender).getLocation());
        sender.sendMessage(AQUA + name + " spawn set: " + ((Entity)sender).getLocation().
                toString());
        return true;
    }
